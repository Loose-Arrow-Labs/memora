...
- `Memora.Core`: domain rules, lifecycle, validation diagnostics, approval queue and workflow rules, diffs, controlled automation policy and safety models, shared agent contracts, imported readiness contract shapes, state-view serialization, and the external runtime contract definition
...
- `Memora.Api`: minimal HTTP host over the shared agent interaction service, imported project readiness lookup, companion OpenAPI document, guarded file-backed session-summary write prototype, and runtime-facing prototype coverage
