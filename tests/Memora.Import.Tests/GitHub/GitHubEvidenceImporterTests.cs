...
public sealed class GitHubEvidenceImporterTests : IDisposable
{
    ...
    [Fact]
    public void Import_NullOptionalBadRecord_PersistsValidAndDiagnostic()
    {
        var workspacePath = CreateWorkspaceWithGitHubAttachment("memora");
    ...
    public void GitHubCli_MalformedPayload_MarksPartialSnapshot(string issuesPayload, string diagnosticCode)
    {
        var runner = new FakeGhRunner(
            new Dictionary<string, string>(StringComparer.Ordinal)
            {
